...
const logDecorator = (value, args) => {
  ...
  if (kind === 'class') {
    console.log('Class decorator!'); // If it's a class decorator, create a new wrapper and return it

    const wrappedClass = class extends value {
      constructor(...args) {
        super(...args);
...
[_initProto, _Person, _initClass] = _applyDecs(Person, [[logDecorator, 2, "intro"]], [logDecorator]);

_initClass();

const p = new _Person('Alex');
p.intro();